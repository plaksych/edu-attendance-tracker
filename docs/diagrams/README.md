...
`run check` заново парсит Mermaid, сверяет source/SVG/PNG hashes с manifest,
сравнивает подписи и структуру нового SVG с сохранённым и проверяет реальные
PNG-пиксели на пустоту. Геометрия и переносы зависят от шрифтов ОС, поэтому
побайтовое совпадение нового рендера между macOS и Linux не требуется.
При расхождении содержимого проверка завершается с ошибкой, не обновляя файлы.
`npm --prefix docs/diagrams test` дополнительно меняет шрифт и проверяет отказ
при подмене source/SVG/PNG, исчезновении текста/линии и пустом изображении.
Оба шага включены в CI; фактический результат указан в матрице готовности.
