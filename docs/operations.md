...
| `MAX_SAMPLED_FRAMES` | верхняя граница кадров из одного ролика | `180` |
| `EVALUATION_TOLERANCE_PEOPLE` | допустимая ошибка для сводки с ручной разметкой | `1` |
...
Для проверки точности не меняйте эталон после обработки: укажите ручное число
людей вместе с загрузкой, затем анализируйте `/recognition/evaluation/summary`.
Сводка не включает материалы без эталона и поэтому не подменяет проверку
витринными примерами.
