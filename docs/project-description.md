...
> Архивное описание камерного прототипа на ревизии `cb34254`. Не инструкция
> по запуску текущей ветки. Актуальные компоненты, upload-only режим и гарантии
> обработки описаны в [архитектуре](architecture.md), настройка - в
> [эксплуатации](operations.md). Указанная ниже ёмкость камер не подтверждена
> нагрузочным тестом текущего кандидата.
