...
def capture_media_links(db: DbSession, capture_id: int) -> dict:
    ...
    if capture.original_object_key and capture.original_bucket:
        ...
        expires_at = utc(uploaded_at) + timedelta(days=settings.original_retention_days)
        if now >= expires_at:
