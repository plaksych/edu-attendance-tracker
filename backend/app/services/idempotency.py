...
def reserve(db, owner_id, route, key, fingerprint):
    if not key or len(key) > 120 or not key.isascii():
        raise HTTPException(422, "Нужен заголовок Idempotency-Key длиной 1–120 ASCII-символов")
    now = datetime.now(timezone.utc)
    insert = pg_insert if db.bind.dialect.name == "postgresql" else sqlite_insert
    db.execute(insert(IdempotencyRecord).values(owner_id=owner_id, route=route, key=key,
        fingerprint=fingerprint, expires_at=now + timedelta(hours=24))
        .on_conflict_do_nothing(index_elements=["owner_id", "route", "key"]))
    record = db.scalar(select(IdempotencyRecord).where(IdempotencyRecord.owner_id==owner_id,
        IdempotencyRecord.route==route, IdempotencyRecord.key==key).with_for_update())
