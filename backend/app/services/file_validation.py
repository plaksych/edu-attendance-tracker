        ...
        with warnings.catch_warnings():
            ...
            with Image.open(stream) as image:
                if image.format != formats[suffix]:
                    raise ValueError("Содержимое не соответствует расширению изображения")
...
def validate_video(stream, suffix):
    ...
    stream.seek(0)
    valid = ((suffix in {".mp4", ".mov"} and header[4:8] == b"ftyp")
    ...
    with tempfile.NamedTemporaryFile(suffix=suffix) as source:
        ...
        try:
            result = subprocess.run(["ffprobe", "-v", "error", "-protocol_whitelist", "file",
                "-probesize", "5000000", "-analyzeduration", "5000000", "-select_streams", "v:0",
                "-show_entries", "stream=codec_name,width,height:format=duration", "-of", "json", source.name],
                capture_output=True, timeout=settings.upload_probe_timeout_seconds, check=True)
            info = json.loads(result.stdout)
            video = info["streams"][0]
            width, height = int(video["width"]), int(video["height"])
            duration = float(info["format"]["duration"])
        except (FileNotFoundError, subprocess.SubprocessError, ValueError, KeyError, IndexError) as exc:
            raise ValueError("Не удалось проверить видео; требуется корректный файл и ffprobe") from exc
        if (width <= 0 or height <= 0 or max(width,height) > settings.upload_max_video_dimension
                or width * height > settings.upload_max_pixels):
            raise ValueError("Разрешение видео превышает лимит")
        if not math.isfinite(duration) or not 0 < duration <= settings.upload_max_duration_seconds:
...
def validate_workbook(stream):
    try:
        with ZipFile(stream) as archive:
            ...
            cells = 0
            for item in files:
                if item.flag_bits & 1 or "vbaProject" in item.filename or "externalLinks/" in item.filename:
                    raise ValueError("Внешние связи, макросы и шифрование XLSX не поддерживаются")
                if item.filename.endswith(".xml"):
                    ...
                    if item.filename.startswith("xl/worksheets/"):
                        ...
                        if len(rows) > 10000 or any(int(r.get("r", "0")) > 10000 for r in rows):
                        ...
                        for cell in root.iter(ns + "c"):
                            ...
                            if column > 200 or cell.find(ns + "f") is not None:
                                raise ValueError("Превышен лимит столбцов или обнаружена формула")
                        if cells > 100000:
