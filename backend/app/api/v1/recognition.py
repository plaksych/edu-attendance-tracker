...
from sqlalchemy.orm import Session as DbSession, selectinload, aliased
...
def create_upload(
    ...
    if session and session.status == SessionStatus.cancelled:
        raise HTTPException(409, "Занятие отменено")
    if measurement_id is not None and measurement.status not in {
        MeasurementStatus.scheduled,
        MeasurementStatus.recognizing,
    }:
        raise HTTPException(
            409, "Замер уже закрыт; загрузите отдельный материал для сравнения"
        )
    db.scalar(select(User).where(User.id == request.state.user.id).with_for_update())
    pending = db.scalar(
        select(func.count(RecognitionJob.id))
        .join(RecognitionUpload)
        .where(
            RecognitionUpload.owner_id == request.state.user.id,
    ...
    if pending >= settings.recognition_pending_per_user:
        raise HTTPException(
            429,
            "Дождитесь обработки ранее загруженных материалов",
            headers={"Retry-After": "30"},
        )
    if measurement_id is not None and measurement.upload:
        raise HTTPException(
            409, "Замер уже связан с материалом; используйте повторную обработку"
        )
...
def get_evaluation_summary(db: DbSession = Depends(get_db)):
    newer_job = aliased(RecognitionJob)
    rows = db.execute(
        select(
        ...
        .where(
            ...
            ~select(newer_job.id)
            .where(
                newer_job.upload_id == RecognitionUpload.id,
                newer_job.id > RecognitionJob.id,
            )
            .correlate(RecognitionUpload, RecognitionJob)
            .exists(),
        )
    ).all()
    absolute_errors = [
        int(row.absolute_error) for row in rows if row.absolute_error is not None
    ]
    relative_errors = [
        float(row.relative_error) for row in rows if row.relative_error is not None
    ]
...
def list_uploads(limit: int = 50, offset: int = 0, db: DbSession = Depends(get_db)):
    ...
    return db.scalars(
        _uploads_query()
        .order_by(RecognitionUpload.id.desc())
        .limit(safe_limit)
        .offset(max(0, offset))
    ).all()
...
def capabilities():
    return {
        ...
        "formats": ["jpg", "jpeg", "png", "webp", "mp4", "mov", "avi", "webm"],
        "max_size_bytes": settings.recognition_upload_max_size_mb * 1024 * 1024,
        "max_pixels": settings.upload_max_pixels,
        "max_duration_seconds": settings.upload_max_duration_seconds,
        "max_video_dimension": settings.upload_max_video_dimension,
        "sample_rate_fps": {"min": 0.1, "max": 10},
        "confidence": {"min": 0.05, "max": 0.95},
    }


@router.post(
    "/uploads/{upload_id}/retry", response_model=RecognitionUploadRead, status_code=202
)
def retry(upload_id: int, request: Request, db: DbSession = Depends(get_db)):
    upload = db.scalar(
        select(RecognitionUpload)
        .where(RecognitionUpload.id == upload_id)
        .with_for_update()
    )
    if not upload:
    ...
    record = reserve(
        db,
        request.state.user.id,
        f"recognition/{upload_id}/retry",
        request.headers.get("Idempotency-Key"),
        hashlib.sha256(str(upload_id).encode()).hexdigest(),
    )
    if record.resource_id is not None:
        return _get_upload(upload_id, db)
    if upload.session_id is not None:
        session = db.scalar(
            select(Session).where(Session.id == upload.session_id).with_for_update()
        )
        if session and session.status == SessionStatus.cancelled:
            raise HTTPException(409, "Занятие отменено")
    ...
    job = RecognitionJob(
        ...
        sample_rate_fps=old.sample_rate_fps,
...
@router.get("/uploads/{upload_id}/history", response_model=RecognitionHistoryRead)
def history(upload_id: int, db: DbSession = Depends(get_db)):
    ...
    corrections = db.scalars(
        select(RecognitionCorrection).where(RecognitionCorrection.job_id.in_(ids))
    )
    return {
        "jobs": [RecognitionUploadJobRead.model_validate(j) for j in upload.jobs],
        "corrections": [
            {
                k: getattr(c, k)
                for k in (
...
def correct(
    ...
    return {
        ...
        "people_count": correction.people_count,
    }
