...
class UserRead(BaseModel):
    id: int
    username: str
    role: Literal["admin", "operator", "teacher", "analyst"]
    enabled: bool
...
class SystemRead(BaseModel):
    ...
    recognition_queue: dict[str, int]
    last_job_heartbeat: datetime | None
    timezone: str
    environment: str
    camera_enabled: bool
    backup_status: Literal["not_verified"]
...
class UserUpdate(BaseModel):
    role: str | None = Field(
        default=None, pattern=r"^(admin|operator|teacher|analyst)$"
    )
...
def throttle(db, key):
    ...
    if now - utc(entry.window_start) >= timedelta(
...
def update_user(
    ...
    if user_id == request.state.user.id and (
        payload.enabled is False or payload.role not in {None, "admin"}
    ):
...
def system_status(db: Session = Depends(get_db)):
    ...
    try:
        ...
        storage_ready = False
    states = db.execute(
        select(RecognitionJob.status, func.count(RecognitionJob.id)).group_by(
    ...
    return {
        ...
        "camera_enabled": bool(
            settings.camera_allowed_cidrs and settings.camera_encryption_key
        ),
