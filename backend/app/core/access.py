...
from app.models import (AttendanceRecord, CameraCapture, Classroom, Discipline, Group,
                        Measurement, RecognitionJob, RecognitionResult, RecognitionUpload,
    ...
    for action, objects in (("create", db.new), ("update", db.dirty), ("delete", db.deleted)):
        for obj in list(objects):
            if obj.__class__.__module__.endswith(".security"):
                continue
            if action == "update" and not db.is_modified(obj, include_collections=False):
                continue
            db.add(AuditEvent(actor_id=request.state.user.id, action=action,
                object_type=obj.__tablename__, object_id=str(getattr(obj, "id", "new")),
...
def restrict_teacher_reads(state):
    ...
    measurement_ids = select(measurements.c.id).where(measurements.c.session_id.in_(session_ids))
    capture_ids = select(captures.c.id).where(captures.c.measurement_id.in_(measurement_ids))
    upload_scope = or_(uploads.c.owner_id == state.session.info["user_id"],
                       uploads.c.session_id.in_(session_ids))
    upload_ids = select(uploads.c.id).where(upload_scope)
    job_scope = or_(jobs.c.camera_capture_id.in_(capture_ids), jobs.c.upload_id.in_(upload_ids))
    criteria = [
        (Group, Group.id.in_(groups)), (Schedule, Schedule.group_id.in_(groups)),
        (Session, Session.id.in_(session_ids)), (Measurement, Measurement.id.in_(measurement_ids)),
        ...
        (RecognitionResult, RecognitionResult.recognition_job_id.in_(select(jobs.c.id).where(job_scope))),
        (AttendanceRecord, AttendanceRecord.session_id.in_(session_ids)),
        (Teacher, Teacher.id.in_(select(schedule.c.teacher_id).where(schedule.c.group_id.in_(groups)))),
        (Classroom, Classroom.id.in_(select(schedule.c.classroom_id).where(schedule.c.group_id.in_(groups)))),
        (Discipline, Discipline.id.in_(select(schedule.c.discipline_id).where(schedule.c.group_id.in_(groups)))),
    ]
