...
def cleanup_orphans(connection, client=None, limit: int = 500) -> int:
    client = client or Minio(settings.minio_endpoint, access_key=settings.minio_access_key,
        ...
        for obj in client.list_objects(settings.minio_bucket, prefix=prefix, recursive=True,
            ...
            with Session(bind=connection) as db:
                if kind == "annotated/jobs":
                    protected = db.scalar(text("""SELECT EXISTS (
                        ...
                        {"bucket": settings.minio_bucket, "key": obj.object_name,
                         "id": int(job_id), "token": token})
                else:
                    protected = db.scalar(text("""SELECT EXISTS (
