...
from app.models import (AccessGrant, AuditEvent, Group, Discipline, Teacher, Classroom,
                        Schedule, Session, RecognitionUpload, RecognitionJob, User)
...
def world():
    ...
    with factory() as db:
        users = [User(username=role, role=role, password_hash=encoded) for role in ("admin","operator","teacher","analyst")]
        db.add_all(users)
        db.flush()
        for number in (1,2):
            group = Group(name=f"G{number}", students_count=20)
            schedule = Schedule(group=group, discipline=Discipline(name=f"D{number}"),
                teacher=Teacher(full_name=f"T{number}"), classroom=Classroom(number=f"R{number}"),
                weekday=3, starts_at=time(9), ends_at=time(10))
            session = Session(schedule=schedule, date=date(2026,9,23), expected_count_snapshot=20)
            db.add(session)
            db.flush()
            db.add(RecognitionUpload(filename=f"image{number}.png", media_type="image", original_bucket="private",
                original_object_key=f"{number}.png", content_type="image/png", size_bytes=100,
                owner_id=users[1].id, session_id=session.id, jobs=[RecognitionJob()]))
...
def test_openapi_and_anonymous_denials(world):
    ...
    for path in ("groups", "schedule", "sessions/today", "recognition/uploads", "recognition/evaluation/summary",
                 "recognition/uploads/1/media", "captures/1/media", "stats/summary", "admin/users"):
...
def test_csrf_role_denial_logout_and_revocation(world):
    ...
    assert client.post("/api/v1/groups", json={"name":"forbidden"}, headers=headers).status_code == 403
...
def test_analyst_cannot_read_media_or_recognition(world):
    ...
    for path in ("recognition/uploads", "recognition/uploads/1/media", "captures/1/media", "cameras", "sessions/1"):
...
def test_admin_mutation_is_audited_without_password(world):
    ...
    with factory() as db:
        ...
        assert all(s.token_hash != client.cookies.get("attendance_session") for s in stored)


def test_request_size_before_parser_and_safe_validation_error(world):
    client, _ = world
    response = client.post("/api/v1/auth/login", content=b"x", headers={"Content-Length":"999999999"})
