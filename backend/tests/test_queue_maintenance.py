...
class MaintenanceUnitTests(unittest.TestCase):
        ...
        for minutes in (1, 10, 20, 29, 30):
            ...
            with self.subTest(minutes=minutes):
                ...
                self.assertLessEqual(
                    abs((first - starts) - timedelta(minutes=minutes / 3)),
    ...
    def test_long_session_keeps_configured_offsets(self):
        ...
        self.assertEqual(
            measurement_times(starts, ends, timedelta(minutes=15)),
    ...
    def test_storage_failure_does_not_disable_recovery(self):
        with (
            patch("app.maintenance.Session") as session,
            patch("app.maintenance.reap_expired_leases") as reap,
        ):
            with (
                patch("app.maintenance.fail_stale_pending_captures"),
                patch("app.maintenance.cleanup_orphans", side_effect=OSError),
            ):
                run_tick(object())
    ...
    def test_no_camera_keeps_measurement_available_for_upload(self):
        measurement = SimpleNamespace(
            session=SimpleNamespace(schedule=SimpleNamespace(classroom_id=None)),
            status="scheduled",
            upload=None,
        )
    ...
    def test_cleanup_preserves_referenced_and_live_attempts(self):
        ...
        keys = [
            f"annotated/jobs/{i}/attempts/1/00000000-0000-0000-0000-000000000001.jpg"
            for i in (1, 2, 3)
        ]
        objects = [
            SimpleNamespace(
        ...
        with patch("app.maintenance.Session") as session:
            session.return_value.__enter__.return_value.scalar.side_effect = [
                True,
                True,
                False,
            ]
    ...
    def test_cleanup_never_deletes_recent_objects(self):
        ...
        _scan_positions.clear()
        obj = SimpleNamespace(
            object_name="annotated/jobs/1/attempts/1/00000000-0000-0000-0000-000000000001.jpg",
...
class MaintenancePostgresTests(unittest.TestCase):
    def setUp(self):
        ...
        with self.engine.begin() as connection:
            ...
            for table in ("recognition_jobs", "camera_captures"):
                connection.execute(
                    text(f"""CREATE TABLE {self.schema}.{table} (
    ...
    def test_upload_recovery_without_any_cameras_or_schedule(self):
        with self.engine.connect() as connection:
            ...
            connection.execute(
                text("""INSERT INTO recognition_jobs(status, attempts, lease_until, claim_token)
            ...
            rows = connection.execute(
                text("SELECT status, claim_token FROM recognition_jobs ORDER BY id")
            ).all()
            self.assertEqual(
                rows, [("retry_wait", None), ("failed", None), ("pending", None)]
            )
            self.assertTrue(
                connection.scalar(
                    text("SELECT lease_until > now() FROM recognition_jobs WHERE id=1")
                )
            )
    ...
    def test_second_scheduler_does_no_work_while_lock_held(self):
        ...
        with self.engine.connect() as leader:
            ...
            try:
                run_service(
                    lambda conn: called.append(1), lock, 0.01, stop, self.engine
                )
            finally:
                timer.join()
