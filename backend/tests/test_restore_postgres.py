...
@pytest.mark.skipif(not os.environ.get("RESTORE_TEST_DSN"), reason="Explicit RESTORE_TEST_DSN required")
def test_database_backup_restore(tmp_path):
    ...
    assert url.host in {"localhost", "127.0.0.1"}, "Drill is restricted to isolated localhost PostgreSQL"
    ...
    try:
        ...
        env = {**os.environ, "DATABASE_URL":source.render_as_string(hide_password=False)}
        run([sys.executable,"-m","alembic","upgrade","head"], cwd=root, env=env)
        engine = create_engine(source)
        with engine.begin() as conn:
            conn.execute(text("INSERT INTO groups(name,course,students_count) VALUES ('RESTORE-CHECK',1,23)"))
        engine.dispose()
        path = tmp_path / "database.dump"
        run(["pg_dump", "--format=custom", "--no-owner", "--file",str(path),
             source.set(drivername="postgresql").render_as_string(hide_password=False)])
        run(["pg_restore", "--exit-on-error", "--no-owner", "--dbname",
             target.set(drivername="postgresql").render_as_string(hide_password=False),str(path)])
        engine = create_engine(target)
        with engine.connect() as conn:
            assert conn.scalar(text("SELECT students_count FROM groups WHERE name='RESTORE-CHECK'")) == 23
