...
@pytest.mark.skipif(not os.environ.get("BACKEND_TEST_DSN"), reason="BACKEND_TEST_DSN required")
def test_upgrade_populated_0005_and_schema_drift():
    ...
    try:
        ...
        with engine.begin() as conn:
            ...
            conn.execute(text("""INSERT INTO groups(id,name,course,students_count) VALUES (1,'legacy',1,28);
        ...
        with engine.connect() as conn:
            ...
            assert conn.scalar(text("SELECT expected_count_snapshot FROM sessions WHERE id=1")) == 24
            assert conn.scalar(text("SELECT person_count FROM detection_snapshots")) == 17
    finally:
        with engine.begin() as conn:
