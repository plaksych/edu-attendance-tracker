...
На Linux CI проверены путь HTTP→S3→YOLO→замеры→CSV и совместное восстановление
БД с файлами в отдельное окружение. Повторное распознавание не меняет закрытый
замер. Проверка использует синтетическую пустую сцену и не измеряет точность
в аудиториях. TLS/IAM рабочего сервера, лицензии и внешняя приёмка остаются
условиями выпуска. [Точные результаты и ограничения](docs/production-readiness.md).
